...
已确认使用子进程标准输入/输出传输 UTF-8 JSON Lines，每行一条请求、响应或事件。标准输出只用于协议，标准错误用于进程错误；Tauri 负责服务进程的启动和关闭。当前已实现任务、配置、凭据、对话启动与取消，以及正文和推理事件推送。桌面宿主已接入服务启动、消息转发和退出清理，React 已接入任务列表、连接设置、普通对话与历史展示。
