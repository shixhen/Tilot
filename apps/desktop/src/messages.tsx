...
function Parts({ parts }: { parts: MessagePart[] }) {
  return <>{parts.map((part, index) => part.kind === "reasoning"
    ? <details className="reasoning" key={index}><summary>思考过程</summary><MessageMarkdown text={part.text} /></details>
...
export function TurnMessages({ record }: { record: TurnRecord }) {
  ...
  return <section className="turn" aria-label="对话轮次">
    {content}
    ...
    {record.turn.status !== "completed" && <p className={`turn-status ${record.turn.status === "failed" ? "error" : "muted"}`}>
      {record.turn.status === "running" && <span className="pulse" />} {labels[record.turn.status]}{record.turn.error ? ` · ${record.turn.error}` : ""}
    </p>}
  </section>;
}
